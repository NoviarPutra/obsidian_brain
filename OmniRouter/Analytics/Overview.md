---
tags:
  - omniroute/analytics
  - omniroute/dashboard
total_requests: 31152
success_rate: "97.8%"
last_updated: "2026-09-06T11:30:01.574558+00:00"
---
# 📈 OmniRoute AI Telemetry & Usage Overview

*Synced from OmniRoute VPS database on 2026-09-06 11:30:01 UTC*

## ⚡ Key Metrics

- **Total AI Requests**: `31.2k`
- **Success Rate**: `97.82%`
- **Input Tokens Processed**: `704.35M` (`704,345,722` tokens)
- **Output Tokens Generated**: `3.44M` (`3,441,084` tokens)
- **Cache Tokens Read**: `623.41M`
- **Avg Response Duration**: `2,237 ms`

---

## 🏢 Usage by Provider

| Provider | Total Requests | Input Tokens | Output Tokens | Avg Latency |
| :--- | :--- | :--- | :--- | :--- |
| **`groq`** | `12.2k` | `74.1k` | `4.8k` | `1349 ms` |
| **`antigravity`** | `11.8k` | `676.95M` | `3.23M` | `3877 ms` |
| **`claude`** | `2.8k` | `27.30M` | `208.2k` | `944 ms` |
| **`upstage`** | `1.4k` | `17.9k` | `144` | `407 ms` |
| **`ollama-cloud`** | `1.1k` | `0` | `0` | `734 ms` |
| **`openrouter`** | `948` | `2.7k` | `854` | `860 ms` |
| **`openference-api`** | `592` | `738` | `646` | `2755 ms` |
| **`zenmux`** | `164` | `57` | `128` | `1501 ms` |
| **`AG`** | `44` | `0` | `0` | `9055 ms` |
| **`orcarouter`** | `43` | `0` | `0` | `358 ms` |
| **`openai-compatible-chat-c4de2144-09c1-49df-8c04-a146bd93af68`** | `10` | `0` | `0` | `4968 ms` |
| **`CC`** | `7` | `0` | `0` | `31945 ms` |

---

## 🤖 Top AI Models

| Model | Provider | Requests | Total Tokens | Avg Latency |
| :--- | :--- | :--- | :--- | :--- |
| **`connection-test`** | `groq` | `12.1k` | `0` | `1339 ms` |
| **`gemini-3.7-flash-high`** | `antigravity` | `10.2k` | `654.21M` | `4045 ms` |
| **`connection-test`** | `claude` | `2.6k` | `0` | `6 ms` |
| **`connection-test`** | `upstage` | `1.4k` | `0` | `404 ms` |
| **`connection-test`** | `antigravity` | `1.2k` | `0` | `439 ms` |
| **`connection-test`** | `ollama-cloud` | `1.0k` | `0` | `673 ms` |
| **`connection-test`** | `openrouter` | `864` | `0` | `427 ms` |
| **`connection-test`** | `openference-api` | `538` | `0` | `2577 ms` |
| **`claude-sonnet-4-6`** | `antigravity` | `415` | `25.96M` | `9427 ms` |
| **`claude-sonnet-5`** | `claude` | `246` | `27.51M` | `10765 ms` |
| **`openai/gpt-oss-120b`** | `groq` | `67` | `78.6k` | `3065 ms` |
| **`AG`** | `AG` | `44` | `0` | `9055 ms` |
| **`z-ai/glm-5.2:free`** | `openrouter` | `31` | `1.7k` | `5485 ms` |
| **`qwen3.5:397b`** | `ollama-cloud` | `27` | `0` | `2681 ms` |
| **`model-sync`** | `openrouter` | `18` | `0` | `6157 ms` |
